...
function Plugin( frame, o ){
	...
	var load = this.reload = function(){
		...
		if( itemNav ){
			...
			// Resize slidee
			$slidee.css( o.horizontal ? { width: '10000px' } : { height: slideeSize+'px' } );
	 ...
	 */
	function syncPages(){
		if(rel.activePage-1>=-1){

			
			$(".big li").removeClass("slid");
			if(rel.activePage-1!=-1){
				$(".big li").eq(rel.activePage-1).addClass("slid");
			}
			$(".big li").eq(rel.activePage+1).addClass("slid");
		}
		//alert(rel.activePage)  这里修改
		if (!$pages.length) {
			return;
		}
	...
	this.activate = function( el, noReposition ){
		...
		$("#loadings").show();
		$(".indexlink").css({"background-image":"url("+$items.eq( index ).attr("data-img")+")"});
		$(".indexlink").attr("href",$items.eq( index ).attr("data-src"));
		setTimeout('$("#loadings").hide()',2000);

		//alert(index);
		
		var max_i=$(".big li img").size()-1;

		$(".big li img").each(function(i){
			var imgheight=$(".big li img").eq(i).height();	
			$(".big li img").eq(i).css("margin-top","-"+(imgheight/2)+"px");
		});
		if(index==0){
		
			$(".big li img").eq(max_i).css("margin-top","-"+($(".big li img").eq(max_i).height()/4)+"px");
			$(".big li img").eq(index).css("margin-top","-"+($(".big li img").eq(index).height()/2)+"px");
		}
	...
	(function(){
		...
		o.scrollBy && $scrollSource.bind('DOMMouseScroll.' + namespace + ' mousewheel.' + namespace, function(e){
			...
			if( itemNav ){
				...
				if(nextItem-1>=-1){
					$(".big li").removeClass("slid");
					$(".big li").eq(nextItem-1).addClass("slid");
					$(".big li").eq(nextItem+1).addClass("slid");
				}
				
				//alert(nextItem); 在这里修改
